...
int main(void) {
  ...
  //new variables
  nunchuk_t n;
  int buttonvalue = -1;//for color counter
  int button_state; //A0
  int state = 0;//user select fr -2 to 2
  int play = 0; //game state, play = 1 when start the game, game over reset to 0
  int score = 0;
  int time;
  int yellow = 0; //color appearances
  int blue = 0;
  int red = 0;
  int green = 0;
  int i, j;
  int delaytime;
  int showtime;
  static int played = 0; //Game rounds
  static int wins = 0;
  ...
  */
  while (1) {
    int random; //for count color
    int random2;// for color match
    int count[4]; //color array
    char* match[] = {"Blue", "Red", "Green","Yellow"};
    int value;
    ...
    if (state == 2) {
      f3d_lcd_drawString(10, 40, "Press button", BLACK, WHITE);
      f3d_lcd_drawString(10, 50, "for color matched", BLACK, WHITE);
      f3d_lcd_drawString(30, 120, "<challenging>", RED, WHITE);
      delaytime = 15;
      showtime = 1000;
    }
    if (state == -1) {
	...
	//challenging color matching(state == 2)
	if (state == 2) {
	  button_state = GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_0);
	  random2 = rand() % 4;
	  f3d_lcd_drawString(40, 140, match[random2], BLACK, WHITE);
	  
	  if(button_state){ //user button been pressed.
	    if (random == random2) { 
	    	wins++;
	    	played++;
	    	rc = f_open(&fid, "win.wav", FA_READ);// corect color matching
	    	if (!rc) {
		      struct ckhd hd;
		      uint32_t waveid;
		      struct fmtck fck;
	    	...
	    	played++;
		rc = f_open(&fid, "fail.wav", FA_READ);// wrong color matching
	    	if (!rc) {
		  struct ckhd hd;
		  uint32_t waveid;
		  struct fmtck fck;
	...
	//TO DO: play w/ board (accel)or nunchuk
	
	//choosing the color by using nunchuk
	f3d_nunchuk_read(&n);
	//up
	if ( n.jx < 150 && n.jx > 100 && n.jy > 150 )
	  buttonvalue = blue;
